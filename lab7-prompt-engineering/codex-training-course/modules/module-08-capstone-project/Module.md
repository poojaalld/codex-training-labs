...
Each capstone lives under labs/capstone-<option> with a tiny React front/backseed.
...
4. **Clinical Decision Support System** (labs/capstone-cdss) – React dashboard for symptoms and vitals, Node endpoint that uses Codex to recommend alerts or care plans.
...
- Write prompts for requirements ? code ? tests ? debugging. Capture the conversation in a prompt-log.md inside each lab folder.
- Use Module.md to sketch the workflow, highlight evaluation criteria (prompt clarity, correctness, test coverage), and outline the delivery steps for each option.
